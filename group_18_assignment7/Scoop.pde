class Scoop{
  ...
  void move(){
    ...
    if (!caught){
      ...
      position.x = cone.xpos+offsetCone.x;
      position.y = cone.ypos+offsetCone.y - levelOnStack*30;
      //ellipse(200,cone.ypos+offsetCone.y - (levelOnStack-1)*30,10,10);
    }
  ...
  void catchScoop(){
    ...
    if (dist(collisionPoint.x,collisionPoint.y,bottom.x,bottom.y)<25){
      caught = true;
      //upping the contents;
